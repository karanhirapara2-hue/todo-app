...
function TodoForm({ setTodos, isSubTodo, todoId, closeForm ,userId,parentUser}) {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = { title, description, dueDate, priority,parentUser};
    
    try {
      let res;
      if (isSubTodo) {
        if(userId){
          console.log("todoId before API call:", todoId);
          res = await adminService.createSubTodoAdmin(todoId,payload,userId);
        }
        else{ 
        res = await subTodoService.createSubTodo(todoId, payload);
        }
      } else {
        if(userId){
          console.log(userId);
          res = await adminService.createTodoAdmin(payload,userId);
        }
        else{
  ...
  return (
    <div className="bg-[#ede8e1] rounded-2xl px-6 py-5 shadow-sm mb-6 space-y-5">
      ...
      <div className="flex justify-between items-start">
        <div>
          <h2 className="text-2xl font-semibold text-[#2e2416]">New task</h2>
          <p className="text-sm text-[#9c8f82] mt-0.5">One thoughtful entry at a time.</p>
        </div>
        <button
          onClick={closeForm}
